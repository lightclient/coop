...
pub fn resolve_trust(user_trust: TrustLevel, situation_ceiling: TrustLevel) -> TrustLevel {
    std::cmp::max(user_trust, situation_ceiling)
}

/// Memory stores accessible at a given trust level.
pub fn accessible_stores(trust: TrustLevel) -> Vec<&'static str> {
    match trust {
...
mod tests {
    ...
    fn full_user_in_dm_gets_full() {
        assert_eq!(resolve_trust(TrustLevel::Full, TrustLevel::Full), TrustLevel::Full);
    }

    #[test]
    fn full_user_in_group_gets_familiar() {
        assert_eq!(resolve_trust(TrustLevel::Full, TrustLevel::Familiar), TrustLevel::Familiar);
    }

    #[test]
    fn inner_user_in_dm_gets_inner() {
        assert_eq!(resolve_trust(TrustLevel::Inner, TrustLevel::Full), TrustLevel::Inner);
    }

    #[test]
    fn inner_user_in_group_gets_familiar() {
        assert_eq!(resolve_trust(TrustLevel::Inner, TrustLevel::Familiar), TrustLevel::Familiar);
    }

    #[test]
    fn public_user_always_public() {
        assert_eq!(resolve_trust(TrustLevel::Public, TrustLevel::Full), TrustLevel::Public);
    }
    ...
    fn accessible_stores_full() {
        assert_eq!(accessible_stores(TrustLevel::Full), vec!["private", "shared", "social"]);
    }
