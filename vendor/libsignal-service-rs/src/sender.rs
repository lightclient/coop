        ...
        for _ in 0..4u8 {
            ...
            let send = if let Some(unidentified) = &unidentified_access {
                tracing::info!(
                    ...
                    unidentified_ws_closed = self.unidentified_ws.is_closed(),
                ...
                tracing::info!(
                    ...
                    message_count = messages.messages.len(),
                    devices = ?messages.messages.iter().map(|m| m.destination_device_id).collect::<Vec<_>>(),
                    identified_ws_closed = self.identified_ws.is_closed(),
            ...
            match send {
                ...
                Err(ServiceError::MismatchedDevicesException(ref m)) => {
                    ...
                    for extra_device_id in &m.extra_devices {
                        tracing::info!(
                            "dropping session with extra device {}",
                            extra_device_id
                        );
                        self.protocol_store
                ...
                Err(ServiceError::StaleDevices(ref m)) => {
                    ...
                    for extra_device_id in &m.stale_devices {
                        tracing::info!(
                            "dropping session with stale device {}",
                            extra_device_id
                        );
                        self.protocol_store
                            .delete_service_addr_device_session(
                ...
                Err(ServiceError::ProofRequiredError(ref p)) => {
                    tracing::warn!(?p, "proof required error");
