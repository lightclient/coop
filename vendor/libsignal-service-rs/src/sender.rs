    ...
    S: ProtocolStore + SenderKeyStore + SessionStoreExt + Sync + Clone,
{
    ...
    pub async fn send_message(
        ...
        let content_label = content_body.to_string();
        let message_to_self = recipient == &self.local_aci;
        let supports_sent_transcript =
            matches!(&content_body, ContentBody::DataMessage(_) | ContentBody::EditMessage(_));
        let sync_message =
        ...
        if needs_sync || is_multi_device {
            ...
            if let Some(sync) = if sync_message {
                ...
                .await?;
            } else if supports_sent_transcript {
                error!("could not create sync message from a direct message");
            } else {
                debug!(
                    content = %content_label,
                    "skipping direct sync message for content without sent-transcript support"
                );
            }
        }
    ...
    pub async fn send_message_to_group(
        ...
        if needs_sync_in_results || self.is_multi_device().await {
                ...
                )
            {
                ...
                debug!(
                    content = %content_label,
                    "skipping group sync message for content without sent-transcript support"
                );
            }
        }
